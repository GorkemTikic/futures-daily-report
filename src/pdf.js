...
const STYLE = `
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* On screen (the website's document view) the @page print margins don't apply,
     so give the sheet its own padding AND a larger, comfortable reading size —
     the 12px base is tuned for A4 print and reads too small on a monitor.
     Page-breaks show as a thin rule instead of a new sheet. */
  @media screen {
    body { font-size:14px; padding:44px 56px; }
    h1 { font-size:38px; }
    h2 { font-size:22px; }
    .lead { font-size:16px; }
    ...
    .define p, .news .note, .news .h, .callout p, .tldr p { font-size:14px; }
    .macro .m .chg { font-size:22px; }
    .page { padding-bottom:30px; margin-bottom:30px; border-bottom:1px solid var(--line2); }
