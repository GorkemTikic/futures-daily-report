...
// Europe/Istanbul, and splits into today vs the rest of the week. Never prints a US time.
...
function istanbulParts(iso) {
  ...
  // then we format in Istanbul.
  const d = new Date(iso);
  if (isNaN(d)) return null;
  const fmt = new Intl.DateTimeFormat("en-GB", {
    timeZone: "Europe/Istanbul", year: "numeric", month: "short", day: "2-digit",
    hour: "2-digit", minute: "2-digit", weekday: "short", hour12: false,
  });
  const parts = Object.fromEntries(fmt.formatToParts(d).map((p) => [p.type, p.value]));
  const dayKey = new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Istanbul", year: "numeric", month: "2-digit", day: "2-digit" }).format(d);
  return { ms: d.getTime(), dayKey, time: `${parts.hour}:${parts.minute}`, label: `${parts.weekday} ${parts.day} ${parts.month}, ${parts.hour}:${parts.minute}` };
}
...
export async function collectCalendar(nowMs = Date.now()) {
  ...
  const todayKey = new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Istanbul", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(nowMs));
  ...
  for (const e of raw) {
    ...
    if (!keep) continue;
    const ist = istanbulParts(e.date);
    if (!ist) continue;
    kept.push({
      title: e.title, impact, when: ist.label, time: ist.time, dayKey: ist.dayKey, ms: ist.ms,
